...
class ThinkUp_Update_Theme_API_Class {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'alante' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'alante' );
...
//			$this->ame_plugin_name 			= get_stylesheet();
			 ...
			 */
			$this->ame_software_product_id = __( $this->ame_product_id, 'alante' );
...
//					$this->ame_plugin_name,
	...
	public static function thinkup_update_theme_inactive_notice() { ?>
		...
		<div id="message" class="error">
			<p><?php printf( __( 'The %s API License Key has not been activated, so the theme is inactive! %sClick here%s to activate the license key and the theme.', 'alante' ), self::theme_name( true ), '<a href="' . esc_url( admin_url( 'options-general.php?page=' . self::theme_name() . '_dashboard' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'alante' ), $this->ame_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
