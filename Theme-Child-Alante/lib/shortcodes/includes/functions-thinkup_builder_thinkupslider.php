...
function thinkup_inputpb_sliderstandard( $instanceID, $image, $video, $title, $description, $link, $button, $class, $style ) {
	...
	foreach ( (array) $image as $increment=>$context ) {
		...
		if ( ! empty( $slide_link ) and empty( $slide_button ) ) {
			$slide_link_start = '<a href="' . $slide_link . '" aria-label="slide link">';
			$slide_link_end   = '</a>';
		} else {
		...
		echo '<li>',
			...
			if ( ! empty( $slide_link ) and ! empty( $slide_button ) ) {
				...
				echo '<div class="featured-link">',
					 '<a href="' . $slide_link . '" aria-label="slide button"><span>' . $slide_button . '</span></a>',
					 '</div>';
			}
		echo '</div>',
			  '</div>',
...
global $wp_embed;
	...
	foreach ( (array) $image as $increment=>$context ) {
		...
		if ( ! empty( $slide_link ) and ! empty( $slide_button ) ) {
			...
			$output_text .= '<a href="' . $slide_link . '" aria-label="slide button"><span>' . $slide_button . '</span></a>';
			$output_text .= '</div>';
		}
	$output_text .= '</div>';
